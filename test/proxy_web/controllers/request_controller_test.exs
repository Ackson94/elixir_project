defmodule ProxyWeb.RequestControllerTest do
  ...
  def fixture(:request) do
    {:ok, request} = Transactions.create_request(@create_attrs)
    request
  end

  describe "index" do
    test "lists all tbl_request", %{conn: conn} do
      conn = get(conn, Routes.request_path(conn, :index))
      assert html_response(conn, 200) =~ "Listing Tbl request"
    end
  end
